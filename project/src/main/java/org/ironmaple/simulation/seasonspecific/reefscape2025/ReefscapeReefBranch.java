...
public class ReefscapeReefBranch extends Goal {
   ...
   *
   * <p>This configures the tolerance used for rotation validation across all reef branch instances.
   * Changes take effect immediately for all instances, including those already created.
   *
  ...
  public static final Translation2d[] branchesCenterPositionBlue =
      new Translation2d[] {
  ...
  public static final Translation2d[] branchesCenterPositionRed =
      Arrays.stream(branchesCenterPositionBlue)
          .map(FieldMirroringUtils::flip)
          .toArray(Translation2d[]::new);

  public static final Rotation2d[] branchesFacingOutwardsBlue =
      new Rotation2d[] {
  ...
  public static Translation3d getPoseOfBranchAt(boolean isBlue, int level, int col) {
    Translation3d branchCenterPosition =
        isBlue
    ...
    return branchCenterPosition
        .plus(heights[level])
        .plus(
            new Translation3d(0.255, 0, 0)
                .rotateBy(new Rotation3d(branchesFacingOutwardsBlue[col])));
  }

  public static final Rotation2d[] branchesFacingOutwardsRed =
   ...
   *
   * <p>This method is called during construction and whenever the static angle tolerance is
   * changed.
   */
      ...
      case 1, 2 -> {
        Rotation3d targetAngle =
            new Rotation3d(
                Degrees.zero(),
                Degrees.of(-35),
                (isBlue ? branchesFacingOutwardsBlue : branchesFacingOutwardsRed)
                    [column].getMeasure());
        setNeededAngle(targetAngle, angleTolerance);
      }
      case 3 ->
          setNeededAngle(
              new Rotation3d(Degrees.zero(), Degrees.of(90), Degrees.zero()), angleTolerance);
  ...
  protected void addPoints() {
    System.out.println(
        "Coral scored on level: " + (level + 1) + " on the " + (isBlue ? "Blue " : "Red") + "reef");
    arena.addValueToMatchBreakdown(
        isBlue, "Auto/CoralScoredInAuto", DriverStation.isAutonomous() ? 1 : 0);
    arena.addValueToMatchBreakdown(isBlue, "CoralScoredOnLevel " + String.valueOf(level + 1), 1);

    int points =
        DriverStation.isAutonomous() ? AUTO_POINTS_BY_LEVEL[level] : TELEOP_POINTS_BY_LEVEL[level];
  ...
  public void draw(List<Pose3d> drawList) {
    ...
    if (this.gamePieceCount > 1) {
      drawList.add(
          getPose().transformBy(new Transform3d(0, isBlue ? 0.12 : -0.12, 0, new Rotation3d())));
    }
  }
}
