...
public class Arena2024Crescendo extends SimulatedArena {
   ...
   *
   * This function returns true during autonomous since the autonomous mode behaves exactly like
   * amplified game play.
   *
   ...
   *
   *
   * <h2>Adds a charge to the amp of the specified team. One charge is equal to one note in the amp
   * so two charges are needed to use the amp.</h2>
   *
  ...
  public boolean activateAmp(boolean isBlue) {
    ...
    System.out.println(
        isBlue
            ...
            : "Red "
                + "Only has "
                + (isBlue ? blueAmpCount : redAmpCount)
                + "of needed 2 to use amp");
    return false;
  }
}
