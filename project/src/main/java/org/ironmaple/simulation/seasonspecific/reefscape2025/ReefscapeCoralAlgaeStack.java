 ...
 *
 * <p>This class represents a CORAL-ALGAE stack, which consists of a standing CORAL with an ALGAE
 * placed on top of it.
 *
 * <p>Three such stacks are staged on each side of the field before the match begins.
 *
 * <p>The stacks will collapse if hit, separating into a {@link ReefscapeCoralOnField} and a {@link
 * ReefscapeAlgaeOnField}.
 *
 * <p>Intakes can also grab a Coral and/or an Algae from the stack.
 *
 * <p>To visualize these stacks, use {@link #getStackedAlgaePoses()} and {@link
 * #getStackedCoralPoses()} to retrieve the positions of the coral and algae in the stacks, and
 * display them through AScope.
 */
public class ReefscapeCoralAlgaeStack extends GamePieceOnFieldSimulation {
  public static final GamePieceInfo REEFSCAPE_STACK_INFO =
      new GamePieceInfo(
          "CoralAlgaeStack", new Circle(0.15), Meters.zero(), Kilograms.of(0.7), 1.8, 4, 0.3);
  ...
  private void throwCoralToGround() {
    ReefscapeCoralOnField coral =
        new ReefscapeCoralOnField(
            new Pose2d(
                stackPosition().plus(new Translation2d(0.15, 0).rotateBy(velocityDirection())),
                velocityDirection()));
   ...
   *
   * @return list of {@link Pose3d} representing the positions of the Coral pieces in all stacks in
   *     the arena
   */
   ...
   *
   * @return a list of {@link Pose3d} representing the positions of the Algae pieces in all stacks
   *     in the arena
   */
  ...
  private static final Transform3d STACK_TO_ALGAE =
      new Transform3d(
          new Translation3d(0, 0, 0.3 + edu.wpi.first.math.util.Units.inchesToMeters(8)),
          new Rotation3d());
